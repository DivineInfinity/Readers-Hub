...
const standardServices=require('../services/standardServices')
exports.insertBookInfo= async function(req,res,next){

    var fetchedBook = await bookFetchService.fetchBookDetails(req.params.x);
  
    // var arr= await standardServices.clone(fetchedBook.items);
    // var response= await standardServices.clone(arr[0]);
    ...
    genere:response.volumeInfo.categories[0],
